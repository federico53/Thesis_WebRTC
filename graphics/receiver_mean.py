...
# Convertire bytesReceived e headerBytesReceived da byte a kilobyte
all_receiver_data['bytesReceived'] = all_receiver_data['bytesReceived'] / (1024)
all_receiver_data['headerBytesReceived'] = all_receiver_data['headerBytesReceived'] / (1024)
...
metrics_to_transform = ['packetsReceived']

for metric in metrics_to_transform:
    mean_receiver_data_per_second[metric] = mean_receiver_data_per_second[metric].diff().fillna(0)
...
ax2 = ax1.twinx()
ax2.plot(mean_cpu_data_per_second.index, mean_cpu_data_per_second['CPU'], color='r', label='CPU Percent')
